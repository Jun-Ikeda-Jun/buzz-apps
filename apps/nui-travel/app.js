...
async function handleFile(file) {
  ...
  try {
    ...
    nuiImage = await blobToImage(blob);
    renderScene();
...
function renderScene() {
  ...
  selectedTemplate.render(ctx, W, H, nuiImage);
}
...
btnDownload.addEventListener("click", async () => {
  ...
  downloadBlob(blob, `nui-travel-${selectedTemplate.id}.png`);
});

btnShare.addEventListener("click", () => {
  const url = twitterShareUrl(
    `ぬいと${selectedTemplate.name}に来ました！🧸✈️\n#NuiTravel #ぬい旅`,
  );
  btnShare.href = url;
});
