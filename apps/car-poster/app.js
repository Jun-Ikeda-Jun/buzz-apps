...
async function handleFile(file) {
  ...
  try {
    ...
    carImage = await blobToImage(blob);
    renderPoster();
...
function renderPoster() {
  ...
  selectedTemplate.render(ctx, W, H, carImage);
}
...
btnDownload.addEventListener("click", async () => {
  ...
  downloadBlob(blob, `car-poster-${selectedTemplate.id}.png`);
});

btnShare.addEventListener("click", () => {
  const url = twitterShareUrl(
    `愛車が映画ポスターになりました！🚗🎬\n#CarPosterMaker #愛車ポスター`,
  );
  btnShare.href = url;
});
